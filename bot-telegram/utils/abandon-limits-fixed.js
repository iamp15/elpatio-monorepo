...
class AbandonLimitManager {
  ...
  canAbandon(telegramId) {
    ...
    console.log(`📊 [CANABANDON] Datos del usuario:`, userData);

    if (!userData) {
      console.log(`✅ [CANABANDON] Usuario sin historial, puede abandonar`);
  ...
  registerAbandonVoluntario(telegramId) {
    ...
    console.log(`📊 [REGISTERABANDON] Datos existentes del usuario:`, userData);
  ...
  getPlayerStats(telegramId) {
    ...
    const userData = this.cache.get(telegramId);
    console.log(`📊 [GETPLAYERSTATS] Datos del usuario:`, userData);

    if (!userData) {
      console.log(`❌ [GETPLAYERSTATS] Usuario no encontrado en cache`);
      return null;
    }
